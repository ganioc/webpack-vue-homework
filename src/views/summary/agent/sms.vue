...
import { getAgentSmsHistory } from '@/api/user'

export default {
  name: 'AgentSmsHistory',
  components: {},
  data() {
    return {
      tableData: [],
  ...
  mounted: function() {
    console.log('agentsmshistory mounted')
    this.getSmsHistory(this.currentPage, this.numPerPage)
  },
  methods: {
    handleCurrentPageChange(val) {
      console.log(`当前页: ${val}`)
      // to load the new page
      this.getSmsHistory(val, this.numPerPage)
      this.currentPage = val
    },
    handlePageSizeChange(val) {
      console.log(`每页 ${val} 条`)
    },
    getSmsHistory(curPage, numPerPage) {
      console.log('getAgentSmsHistory')
      console.log(curPage, numPerPage)
      getAgentSmsHistory(curPage, numPerPage).then(
        response => {
        ...
        err => {
          console.log(err)
          console.log('getAgentSmsHistory failed')
        }
      )
    }
  }
